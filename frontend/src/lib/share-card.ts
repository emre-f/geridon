...
export function renderBacktestCard(run: BacktestRunRecord, strategyName: string) {
  ...
  context.textAlign = "right";
  context.fillText(`ran ${formatCardDate(Date.parse(run.created_at + "Z") || Date.now())}`, shareCardWidth - padding, padding + 12);

  // Strategy name + setup line.
  context.textAlign = "left";
  context.fillStyle = palette.ink;
  context.font = shareCardFont(700, 40);
  context.fillText(strategyName, padding, padding + 78, shareCardWidth - padding * 2 - 300);
  ...
  context.fillText(formatPercent(run.metrics.total_return_pct), shareCardWidth - padding, padding + 92);

  // Equity curve.
  drawEquityCurve(context, run, {
    x: padding,
    y: 236,
    width: shareCardWidth - padding * 2 - 92,
    height: 270,
  });
  ...
  const statsY = shareCardHeight - padding - 34;
  const statWidth = (shareCardWidth - padding * 2) / stats.length;
