...
test("identical config and seed reproduce the same trials and ranking", async () => {
  const first = await runOptimization(curatedThresholdConfig());
  const second = await runOptimization(curatedThresholdConfig());
  assert.deepEqual(
...
test("different seeds explore a different candidate sequence", async () => {
  const first = await runOptimization(curatedThresholdConfig(1));
  const second = await runOptimization(curatedThresholdConfig(2));
  assert.notDeepEqual(
    second.trials.map((trial) => trial.hash),
    first.trials.map((trial) => trial.hash),
  );
...
test("random search finds the best curated thresholds and beats the baseline", async () => {
  const result = await runOptimization(curatedThresholdConfig());
...
test("duplicate candidates are rejected with a recorded reason", async () => {
  const result = await runOptimization(curatedThresholdConfig());
...
test("successive halving prunes weak candidates before the final stage", async () => {
...
test("refinement trials search a narrowed space around top candidates", async () => {
  const result = await runOptimization(baseConfig(thresholdStrategy(95, 105)));
...
test("maxTrials is a hard total budget including refinement and rejected candidates", async () => {
  const maxTrials = 20;
...
test("optional rule toggles let the optimizer disable a harmful rule", async () => {
...
test("the ablation report covers every active rule of the best candidate", async () => {
  const result = await runOptimization(curatedThresholdConfig());
  ...
  const groupedResult = await runOptimization(baseConfig(grouped, { maxTrials: 6 }));
...
test("baseline and buy & hold benchmarks are always evaluated", async () => {
  const result = await runOptimization(curatedThresholdConfig());
