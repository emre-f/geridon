...
test("an experiment runs to completion and persists ranked trials", async () => {
  ...
  assert.equal(experiment.snapshot.search_space_version, 3);
...
test("invalid configurations return structured errors", async () => {
  ...
  const cases: Array<[Record<string, unknown>, number]> = [
    ...
    [experimentBody(strategyId, { worker_count: 0 }), 400],
    [experimentBody(strategyId, { worker_count: 1.5 }), 400],
    [experimentBody(strategyId, { worker_count: 100_000 }), 400],
