...
function backtestRunRecord(row: Record<string, unknown>): BacktestRunRecord {
  const detail = JSON.parse(String(row.detail)) as Pick<
    BacktestResult,
    "equity_curve" | "trades"
  >;
  return {
...
function handleRunBacktest(body: unknown) {
  if (!body || typeof body !== "object" || Array.isArray(body)) {
    return badRequest("Request body must be an object.");
  }
  ...
  // Always-in mode is stop-and-reverse: every fill flips the whole account,
  // so partial sizing does not apply.
  const buyPercent =
...
function handleListStrategyBacktests(idPath: string) {
  const id = parseStrategyId(idPath);
  if (id == null) {
    return badRequest("Strategy id must be a positive integer.");
  }
...
function handleGetBacktest(idPath: string) {
  const id = parseStrategyId(idPath);
  if (id == null) {
...
function handleDeleteBacktest(idPath: string) {
  const id = parseStrategyId(idPath);
  if (id == null) {
    return badRequest("Backtest id must be a positive integer.");
  }
...
const server = createServer(async (request, response) => {
  try {
    ...
    if (request.method === "POST" && url.pathname === "/api/v1/backtests") {
      const result = handleRunBacktest(await readJson(request));
      sendJson(response, result.statusCode, result.body);
      return;
    }
    ...
    if (request.method === "GET" && strategyBacktestsMatch) {
      const result = handleListStrategyBacktests(strategyBacktestsMatch[1]);
      sendJson(response, result.statusCode, result.body);
      return;
    }
    ...
    if (backtestMatch) {
      if (request.method === "GET") {
        const result = handleGetBacktest(backtestMatch[1]);
        sendJson(response, result.statusCode, result.body);
        return;
      }
      if (request.method === "DELETE") {
        const result = handleDeleteBacktest(backtestMatch[1]);
        sendJson(response, result.statusCode, result.body);
        return;
      }
    }
